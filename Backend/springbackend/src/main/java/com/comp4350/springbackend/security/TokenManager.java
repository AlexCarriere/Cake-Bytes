...
public class TokenManager {
    ...
    private static final long INTERVAL = 60 * 60 * 60 * 1000;  //60 min to initiate request, token failure

    private final Map<String, Long> tokenPool = new HashMap<>();

    // token is:  BASE64-encoded value of username-UUID-time
    public String generateToken(Customer customer) {
        if (customer == null) return null;
        logger.debug("generate a token for user: " + customer.getUsername());
        cleanToken();
        String res = customer.getUsername() + '-' + UUID.randomUUID().toString().replaceAll("-", "")
